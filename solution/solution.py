...
OPENAI_MODEL = os.getenv("LAB_MODEL", "gemini-3.6-flash")
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        ...
        g4 = r.get("gpt4o_answer", "")[:37] + "..." if len(r.get("gpt4o_answer", "")) > 40 else r.get("gpt4o_answer", "")
        mn = r.get("mini_answer", "")[:37] + "..." if len(r.get("mini_answer", "")) > 40 else r.get("mini_answer", "")
        g4_t = f"{r.get('gpt4o_time', 0.0):.2f}s"
        mn_t = f"{r.get('mini_time', 0.0):.2f}s"
        lines.append(f"{p:<40} | {g4:<40} | {mn:<40} | {g4_t:<14} | {mn_t:<12}")
    return "\n".join(lines)
